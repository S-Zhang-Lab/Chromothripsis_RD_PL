...
figures_dir <- file.path(base_dir, "Figures/L2")
...
L2 <- CreateSeuratObject(counts = L2.data$"Gene Expression", 
                             project = "L2")

# Add HTO objects
L2$Abs <- CreateAssayObject(counts = L2.data$"Antibody Capture")
L2.Abs <- t(as.data.frame(L2$"Abs"@counts))

# Subset HTO groups for cell group demultiplexing
L2.Abs <- t(L2.Abs[,c(3,4)])

# Create HTO assay with only R2 reads
L2[["HTO"]] <- CreateAssayObject(counts = L2.Abs) 
rm(L2.Abs)
...
plot4 <- FeatureScatter(L2, feature1 = "nFeature_HTO", feature2 = "nCount_HTO")
CombinePlots(plots = list(plot3, plot4))
dev.off()

# Removing unwanted cells from the data based on above QC plots
L2 <- subset(L2, subset = nFeature_RNA > 200 & nFeature_RNA < 15000 & percent.mt < 25)
...
L2 <- SCTransform(L2, vars.to.regress = "percent.mt", verbose = TRUE)

# finding variable genes
L2 <- FindVariableFeatures(L2, selection.method = "vst", nfeatures = 2000)

# Identify the 10 most highly variable genes
top10 <- head(VariableFeatures(L2), 10)

# plot variable features with and without labels
pdf(file.path(figures_dir, "Variable.Genes.pdf"),width=10,height=4,paper='special')
plot1 <- VariableFeaturePlot(L2)
...
set.seed(1)
L2 <- RunPCA(L2, verbose = FALSE)
pdf(file.path(figures_dir, "Elbowplot.pdf"),width=7,height=5,paper='special')
ElbowPlot(L2)
dev.off()

print(L2[["pca"]], dims = 1:5, nfeatures = 5)


pdf(file.path(figures_dir, "Dim.plot.pdf"),width=5,height=4,paper='special')
DimPlot(L2, reduction = "pca")
dev.off()

# Cluster the cells

L2 <- FindNeighbors(L2, dims = 1:10)
L2 <- FindClusters(L2, resolution = 1)
head(Idents(L2), 5)

# Run non-linear dimensional reduction (UMAP/tSNE)
L2 <- RunUMAP(L2, dims = 1:10, verbose = TRUE)
...
DimPlot(L2, reduction = "umap", label = TRUE, split.by = "hash.ID")
dev.off()
...
DoHeatmap(L2, features = top10$gene) + 
...
ggplot(L2@meta.data, aes(x=hash.ID, fill=seurat_clusters)) + 
...
ggplot(L2@meta.data, aes(seurat_clusters, group = hash.ID)) + 
...
counts_matrix <- as.data.frame(L2@assays$RNA$counts)
meta <- L2@meta.data

# Annotate immune cells based on SingleR output
annot <- as.data.frame(L2@meta.data$hash.ID)
colnames(annot) <- c('V1')
...
infercnv_obj = infercnv::run(infercnv_obj,
                             ...
                             out_dir=file.path(base_dir, "/DATA/InfCNV/L2/CNV_output"),
                             output_format="pdf",
...
infercnv_obj = infercnv::run(infercnv_obj,
                             cutoff=0.1, # cutoff=1 works well for Smart-seq2, and cutoff=0.1 works well for 10x Genomics
                             out_dir=file.path(base_dir, "/DATA/InfCNV/L2/CNV_HMM_output"),
                             output_format="pdf",
                             cluster_by_groups=TRUE, 
                             denoise=TRUE,
                             HMM=TRUE,
...
L2 = infercnv::add_to_seurat(
  infercnv_output_path=file.path(base_dir, "/DATA/InfCNV/L2/CNV_HMM_output"),
  seurat_obj=L2, # optional
  top_n=20
  )
...
FeaturePlot(L2 , features="has_loss_chr3") + 
  ggplot2::scale_colour_gradient(low="lightgrey", high="blue", limits=c(0,1))
FeaturePlot(L2 , features="has_dupli_chr3") + 
  ggplot2::scale_colour_gradient(low="lightgrey", high="blue", limits=c(0,1))


## extract annnotation
HMM_anno <- colnames(L2@meta.data[, 19:257])
Chr3_changes <- c(colnames(L2@meta.data[, 37:45]))
Chr3_changes               
Chr5_changes <- c(colnames(L2@meta.data[, 55:63]))
Chr5_changes               

                  
pdf(file.path(figures_dir, "Featureplot_CNV_3p.pdf"),
    width=12,height=10,paper='special')
FeaturePlot(L2, features = Chr3_changes)
dev.off()

pdf(file.path(figures_dir, "Featureplot_CNV_5q.pdf"),
    width=12,height=10,paper='special')
FeaturePlot(L2, features = Chr5_changes)
dev.off()

pdf(file.path(figures_dir, "Featureplot_CNV_3p_5q.pdf"),
    width=16,height=16,paper='special')
FeaturePlot(L2, features = c(Chr3_changes, Chr5_changes))
dev.off()
...
# Initialize the Seurat object with the raw (non-normalized data).
df <- CreateSeuratObject(counts = L2.data$"Gene Expression", project = "MULTI-Chr")

# add ADT/HTO/BC objects
df$HTO <- CreateAssayObject(counts = L2.data$"Antibody Capture")

L2.HTOs <- t(as.data.frame(df$"HTO"@counts))
L2.HTOs <- t(L2.HTOs[,7:12]) #subset only R2 reads for cell group demultiplexing
df[["HTO"]] <- CreateAssayObject(counts = (L2.HTOs+0.1)*10) # create HTO assay with only R2 reads counts 
df[["MULTI"]] <- CreateAssayObject(counts = (L2.HTOs)) #create MULTI-seq assay based on raw HTO counts with only R2 reads
rm(L2.HTOs)

str(df)

# do MULTIseqDemux on L2 df HTO assay (contains both R1 and R2)
L2 <- MULTIseqDemux(
  L2,
  assay = "HTO",
  quantile = 0.9,
...
table(L2$MULTI_ID)
table(L2$MULTI_classification)

# Re-code ID by combining R1 classification and R2 classification. 
L2@meta.data$MULTIdemuxR1R2 = recode(
  L2@meta.data$MULTI_ID, 
  "R1-BC-ID1" = "BC-ID1", 
  "R2-BC-ID1" = "BC-ID1", 
  "R1-BC-ID2" = "BC-ID2", 
  "R2-BC-ID2" = "BC-ID2",
...
table(L2$MULTIdemuxR1R2)


######################################################
#    select Chr3 cells highest scaled_cnv            # 
######################################################
Idents(L2) <- "seurat_clusters"
DimPlot(L2)

Idents(L2) <- "proportion_scaled_cnv_chr3"
DimPlot(L2)

# Select cells with the highest 3p loss with very stringent cut off (>0.15)
L2.3ploss = subset(L2, idents = "0", invert = TRUE)
L2.3ploss@meta.data$Chr3p.loss <- ifelse(
  L2.3ploss@meta.data$proportion_scaled_cnv_chr3 > 0.15, "TRUE", "FALSE"
  )
meta.3ploss = L2.3ploss@meta.data
table(meta.3ploss$Chr3p.loss)


Idents(L2.3ploss) <- "Chr3p.loss"
DimPlot(L2.3ploss, split.by = "HTO_maxID")
...
Idents(L2) <- "proportion_scaled_cnv_chr5"
DimPlot(L2)

# select cells with highest chr5 cnv with very stringent cut off (>0.2)
L2.5q.gain = subset(L2, idents = "0", invert = TRUE)
L2.5q.gain@meta.data$Chr5q.gain <- ifelse(
  L2.5q.gain@meta.data$proportion_scaled_cnv_chr5 > 0.2, "TRUE", "FALSE"
  )
meta.5q.gain = L2.5q.gain@meta.data
meta.5q.gain$Chr5q.gain

Idents(L2.5q.gain) <- "Chr5q.gain"
DimPlot(L2.5q.gain, split.by = "HTO_maxID")
...
FeaturePlot(L2.3ploss, features = c("GBE1", "ROBO1", "ROBO2", "ZNF717"))
dev.off()
...
FeaturePlot(L2, features = "cnv_avg")
L2.clean <- subset(L2, subset = cnv_avg < 0.05)

L2.clean$cnv.class <- "cnv_avg.normal"
L2.clean$cnv.class[L2.clean$cnv_avg > 0.03] <- "cnv_avg.high"
L2.clean$cnv.class[L2.clean$cnv_avg > 0.01 & L2.clean$cnv_avg <= 0.03] <- "cnv_avg.mid"
L2.clean$cnv.class[L2.clean$cnv_avg > 0.00 & L2.clean$cnv_avg <= 0.01] <- "cnv_avg.low"

L2.clean$HTO_recode <- recode(
  L2.clean$HTO_maxID, 
  "R2-BC-ID1" = "01_ID1", 
  "R2-BC-ID2" = "02_ID2", 
  "R2-BC-ID3" = "03_ID3", 
...
Idents(L2.clean) <- "HTO_recode"
Idents(L2.clean) <- factor(x = Idents(L2.clean), levels = sort(levels(L2.clean))) # sort idents

Idents(L2.clean) <- "HTO_maxID"
Idents(L2.clean) <- factor(x = Idents(L2.clean), levels = sort(levels(L2.clean))) # sort idents

table(L2.clean$cnv.class)
pdf(file.path(figures_dir, "VlnPlot_L2.clean.VHL_cnv.class.pdf"),width=8,height=4,paper='special')
VlnPlot(L2.clean, feature = "VHL", split.by = "cnv.class")
dev.off()

#==  re select Chr3 cells highest scaled_cnv  ==# 

## Identify cells with highest chr3 changes
Idents(L2.clean) <- "seurat_clusters"
DimPlot(L2.clean)

Idents(L2.clean) <- "proportion_scaled_cnv_chr3"
DimPlot(L2.clean)

# Select cells with the highest 3p loss with very stringent cut off (>0.2)
L2.clean.3ploss = subset(L2.clean, idents = "0", invert = TRUE)
Idents(L2.clean.3ploss) <- "proportion_scaled_cnv_chr3"
DimPlot(L2.clean.3ploss)
L2.clean.3ploss@meta.data$Chr3p.loss <- ifelse(
  L2.clean.3ploss@meta.data$proportion_scaled_cnv_chr3 > 0.2, 
  "TRUE", "FALSE"
  )
meta.3ploss = L2.clean.3ploss@meta.data
table(meta.3ploss$Chr3p.loss)
Idents(L2.clean.3ploss) <- "Chr3p.loss"
DimPlot(L2.clean.3ploss)
DimPlot(L2.clean.3ploss, split.by = "HTO_maxID")

pdf(file.path(figures_dir, "DimPlot_L2.clean.3ploss_BC_ID.pdf"),
    width=16,height=4,paper='special')
DimPlot(L2.clean.3ploss, split.by = "HTO_maxID")
dev.off()
...
Idents(L2.clean) <- "proportion_scaled_cnv_chr5"
DimPlot(L2.clean)

# select cells with highest chr5 cnv with very stringent cut off (>0.2)
L2.clean.5q.gain = subset(L2.clean, idents = "0", invert = TRUE)
L2.clean.5q.gain@meta.data$Chr5q.gain <- ifelse(
  L2.clean.5q.gain@meta.data$proportion_scaled_cnv_chr5 > 0.2, 
  "TRUE", "FALSE"
  )
meta.5q.gain = L2.clean.5q.gain@meta.data
meta.5q.gain$Chr5q.gain

Idents(L2.clean.5q.gain) <- "Chr5q.gain"
DimPlot(L2.clean.5q.gain, split.by = "HTO_maxID")

pdf(file.path(figures_dir, "DimPlot_L2.clean.5q.gain_BC_ID.pdf"),
    width=16,height=4,paper='special')
DimPlot(L2.clean.5q.gain, split.by = "HTO_maxID")
dev.off()


Idents(L2.clean.5q.gain) <- "proportion_scaled_cnv_chr5"
pdf(file.path(figures_dir, "DimPlot_L2.clean.5q.gain_proportion_scaled_cnv_chr5.pdf"),
    width=16,height=4,paper='special')
DimPlot(L2.clean.5q.gain, split.by = "HTO_maxID")
dev.off()

# 5q cut site coordinates: 98,852,808. The cutting should happen between these two genes
FeaturePlot(L2.clean.5q.gain, feature = c("CAST", "CHD1")) 

Idents(L2.clean.5q.gain) <- "HTO_maxID"
Idents(L2.clean.5q.gain) <- factor(x = Idents(L2.clean.5q.gain), 
                                       levels = sort(levels(L2.clean.5q.gain))
                                       )
Idents(L2.clean.3ploss) <- "HTO_maxID"
Idents(L2.clean.3ploss) <- factor(x = Idents(L2.clean.3ploss), 
                                      levels = sort(levels(L2.clean.3ploss))
                                      )

pdf(file.path(figures_dir, "VlnPlot_L2.clean.5q.gain_CAST.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean.5q.gain, feature = "CAST", split.by = "proportion_scaled_cnv_chr5")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L2.clean.5q.gain_CHD1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean.5q.gain, feature = "CHD1", split.by = "proportion_scaled_cnv_chr5")
dev.off()


VlnPlot(L2.clean.5q.gain, feature = "VHL", split.by = "proportion_scaled_cnv_chr5")

pdf(file.path(figures_dir, "VlnPlot_L2.clean.5q.gain_VHL_by.proportion_scaled_cnv_chr5.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean, feature = "VHL", split.by = "proportion_scaled_cnv_chr5")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L2.clean.5q.gain_VHL_by.cnv.class.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean, feature = "VHL", split.by = "cnv.class")
dev.off()
...
pdf(file.path(figures_dir, "VlnPlot_L2.clean.proportion_scaled_cnv_chr5_SQSTM1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean, feature = "SQSTM1", split.by = "proportion_scaled_cnv_chr5")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L2.clean.proportion_scaled_cnv_chr3_SQSTM1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean, feature = "SQSTM1", split.by = "proportion_scaled_cnv_chr3")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L2.clean.proportion_scaled_cnv_chr5_VHL.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean, feature = "VHL", split.by = "proportion_scaled_cnv_chr5")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L2.clean.proportion_scaled_cnv_chr3_VHL.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean, feature = "VHL", split.by = "proportion_scaled_cnv_chr3")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L2.clean.has_dupli_chr5_VHL.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean, feature = "VHL", split.by = "has_dupli_chr5")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L2.clean.has_loss_chr3_VHL.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean, feature = "VHL", split.by = "has_loss_chr3")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L2.clean.has_dupli_chr5_SQSTM1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean, feature = "SQSTM1", split.by = "has_dupli_chr5")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L2.clean.has_loss_chr3_SQSTM1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean, feature = "SQSTM1", split.by = "has_loss_chr3")
dev.off()

### good plots ====
pdf(file.path(figures_dir, "VlnPlot_L2.clean.proportion_scaled_dupli_chr5_SQSTM1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean, feature = "SQSTM1", split.by = "proportion_scaled_dupli_chr5")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L2.clean.proportion_scaled_loss_chr3_SQSTM1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean, feature = "SQSTM1", split.by = "proportion_scaled_loss_chr3")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L2.clean.proportion_scaled_loss_chr5_SQSTM1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean, feature = "SQSTM1", split.by = "proportion_scaled_loss_chr5")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L2.clean.proportion_scaled_dupli_chr3_SQSTM1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean, feature = "SQSTM1", split.by = "proportion_scaled_dupli_chr3")
dev.off()


VlnPlot(L2.clean, feature = "BAP1", split.by = "proportion_scaled_dupli_chr5")
VlnPlot(L2.clean, feature = "FANCD2", split.by = "proportion_scaled_dupli_chr5")


# ====  recode chr3.loss and chr5.dupli to simply the plotting

L2.clean$Chr3.loss <- ifelse(
  L2.clean@meta.data$proportion_scaled_loss_chr3 > 0.2, 
  "Chr3.loss_TRUE", "Chr3.loss_FALSE"
  )
L2.clean$Chr5.dupli <- ifelse(
  L2.clean@meta.data$proportion_scaled_dupli_chr5 > 0.2, 
  "Chr5.dupli_TRUE", "Chr5.dupli_FALSE"
  )

Idents(L2.clean) <- "HTO_maxID"
Idents(L2.clean) <- factor(x = Idents(L2.clean), levels = sort(levels(L2.clean)))

pdf(file.path(figures_dir, "VlnPlot_L2.clean.Chr3.loss_SQSTM1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean, feature = "SQSTM1", split.by = "Chr3.loss")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L2.clean.Chr5.dupli_SQSTM1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean, feature = "SQSTM1", split.by = "Chr5.dupli")
dev.off()
...
Idents(L2.clean) <- "Chr3.loss"
Idents(L2.clean) <- factor(x = Idents(L2.clean), levels = sort(levels(L2.clean)))

table(Idents(L2.clean))
DEG_Chr3.loss <- FindMarkers(L2.clean, ident.1 = c('Chr3.loss_TRUE'), ident.2 = c('Chr3.loss_FALSE'), min.pct = 0.25)
write.csv(DEG_Chr3.loss, file = file.path(figures_dir, "DEG_Chr3.loss_DEG.csv"))

pdf(file.path(figures_dir, "Volcano_L2.clean.DEG.Chr3.loss.pdf"),
    width=6,height=4,paper='special')
...
pdf(file.path(figures_dir, "Volcano_L2.clean.DEG.Chr3.loss_INHBA.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean, feature = "INHBA", split.by = "Chr3.loss")
dev.off()

pdf(file.path(figures_dir, "Volcano_L2.clean.DEG.Chr3.loss_SOD3.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean, feature = "SOD3", split.by = "Chr3.loss")
dev.off()

## Identify global DE genes by comparing cell with/without Chr5.dupli
Idents(L2.clean) <- "Chr5.dupli"
Idents(L2.clean) <- factor(x = Idents(L2.clean), levels = sort(levels(L2.clean)))

table(Idents(L2.clean))
DEG_Chr5.dupli <- FindMarkers(L2.clean, ident.1 = c('Chr5.dupli_TRUE'), ident.2 = c('Chr5.dupli_FALSE'), min.pct = 0.25)
write.csv(DEG_Chr5.dupli, file = file.path(figures_dir, "DEG_Chr5.dupli_DEG.csv"))

# draw volcano plot
pdf(file.path(figures_dir, "Volcano_L2.clean.DEG.Chr5.dupli.pdf"),
    width=6,height=4,paper='special')
...
pdf(file.path(figures_dir, "Volcano_L2.clean.DEG.Chr5.dupli_ANKHD1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean, feature = "ANKHD1", split.by = "Chr5.dupli")
dev.off()

pdf(file.path(figures_dir, "Volcano_L2.clean.DEG.Chr5.dupli_TAF7.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean, feature = "TAF7", split.by = "Chr5.dupli")
dev.off()

pdf(file.path(figures_dir, "Volcano_L2.clean.DEG.Chr5.dupli_TARBP2.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean, feature = "TARBP2", split.by = "Chr5.dupli")
dev.off()

pdf(file.path(figures_dir, "Volcano_L2.clean.DEG.Chr5.dupli_G3BP.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L2.clean, feature = "G3BP1", split.by = "Chr5.dupli")
dev.off()
...
Idents(L2.clean) <- "seurat_clusters"
L2.clean.3p5qcnv = subset(L2.clean, idents = "0", invert = TRUE)
L2.clean.3p5qcnv@meta.data$cnv.3p5q <- ifelse(
  L2.clean.3p5qcnv$Chr3.loss == "TRUE" & 
    L2.clean.3p5qcnv$Chr5.dupli == "TRUE", "TRUE", "FALSE"
  )

table(L2.clean.3p5qcnv$Chr3.loss)
table(L2.clean.3p5qcnv$Chr5.dupli)

L2.clean.3p5qcnv.meta <- L2.clean.3p5qcnv@meta.data
filtered_cells <- cbind(L2.clean.3p5qcnv$Chr3.loss, L2.clean.3p5qcnv$Chr5.dupli)
filtered_cells.3p5qcnv.selected <- rownames(
  L2.clean.3p5qcnv.meta)[L2.clean.3p5qcnv.meta$Chr3.loss
                             == "TRUE" & L2.clean.3p5qcnv.meta$Chr5.dupli 
                             == "TRUE"]

# ==== using less stringent cut offs
L2.clean$Chr3loss.Chr5.dupli <- ifelse(
  L2.clean@meta.data$proportion_scaled_loss_chr3 > 0 &
  L2.clean@meta.data$proportion_scaled_dupli_chr5 > 0, 
  "Chr3loss.Chr5.dupli_TRUE", "Chr3loss.Chr5.dupli_FALSE"
  )

table(L2.clean$Chr3loss.Chr5.dupli)
...
kp <- plotKaryotype(plot.type=2, L2somes = c("chr3"))
...
pdf(file.path(figures_dir, "Karyotype_L2.clean.DEG.Chr3.pdf"),
...
pdf(file.path(figures_dir, "Karyotype_L2.clean.DEG.Chr3_top50.pdf"),
    width=30,height=30,paper='special')
...
pdf(file.path(figures_dir, "Karyotype_L2.clean.DEG.Chr3_top50_AllL2.pdf"),
    width=30,height=30,paper='special')
...
# === Only plot Chr3  ===
pdf(file.path(figures_dir, "Karyotype_L2.clean.DEG.Chr3_only_top50.pdf"),
    width=10,height=8,paper='special')

kp <- plotKaryotype(plot.type=2, plot.params = pp, L2somes = c("chr3"))
kpAddMainTitle(kp, main = "Gene expression - Chr3 loss True vs False")
## Data panel 1
kpPoints(kp, data=sign.genes, y=sign.genes$avg_log2FC, cex=cex.val, ymax=fc.ymax, ymin=fc.ymin, r1=points.top, col=sign.col)
gene.mean <- start(top.genes) + (end(top.genes) - start(top.genes))/2
...
# establish karyotype plot
kp <- plotKaryotype(plot.type=2, L2somes = c("chr5"))
kpDataBackground(kp, data.panel = 1, col="#AACBFF")
kpDataBackground(kp, data.panel = 2, col="#FFAACB")
...
pdf(file.path(figures_dir, "Karyotype_L2.clean.DEG.chr5.pdf"),
    width=30,height=30,paper='special')
kp <- plotKaryotype(plot.type=2)
kpDataBackground(kp, data.panel = 1, col="#AACBFF")
kpPoints(kp, data=sign.genes, y=sign.genes$avg_log2FC, ymax=fc.ymax, ymin=fc.ymin)
...
# add top 50 gene symbol
pdf(file.path(figures_dir, "Karyotype_L2.clean.DEG.chr5_top50.pdf"),
    width=30,height=30,paper='special')
...
pdf(file.path(figures_dir, "Karyotype_L2.clean.DEG.Chr5_top50_AllChrom2.pdf"),
    width=30,height=30,paper='special')
...
# === Only plot chr5  ===
pdf(file.path(figures_dir, "Karyotype_L2.clean.DEG.chr5_only_top50.pdf"),
    width=10,height=8,paper='special')

kp <- plotKaryotype(plot.type=2, plot.params = pp, L2somes = c("chr5"))
kpAddMainTitle(kp, main = "Gene expression - chr5 dupli True vs False")
## Data panel 1
kpPoints(kp, data=sign.genes, y=sign.genes$avg_log2FC, cex=cex.val, ymax=fc.ymax, ymin=fc.ymin, r1=points.top, col=sign.col)
gene.mean <- start(top.genes) + (end(top.genes) - start(top.genes))/2
