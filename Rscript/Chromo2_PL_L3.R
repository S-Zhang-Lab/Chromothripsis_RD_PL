...
figures_dir <- file.path(base_dir, "Figures/L3")
...
L3 <- CreateSeuratObject(counts = L3.data$"Gene Expression", 
                             project = "L3")

# Add HTO objects
L3$Abs <- CreateAssayObject(counts = L3.data$"Antibody Capture")
L3.Abs <- t(as.data.frame(L3$"Abs"@counts))

# Subset HTO groups for cell group demultiplexing
L3.Abs <- t(L3.Abs[,c(1,3:7)])
rownames(L3.Abs)
rownames(L3.Abs)[rownames(L3.Abs) == "H-HTO-1.1"] <- "H-HTO-1"

# Create HTO assay 
L3[["HTO"]] <- CreateAssayObject(counts = L3.Abs) 
rm(L3.Abs)
...
plot1 <- FeatureScatter(L3, feature1 = "nCount_RNA", feature2 = "percent.mt")
plot2 <- FeatureScatter(L3, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
CombinePlots(plots = list(plot1, plot2))
dev.off()

pdf(file.path(figures_dir, "Featureplot_HTO.pdf"),width=10,height=4,paper='special')
plot3 <- FeatureScatter(L3, feature1 = "nFeature_HTO", feature2 = "nCount_HTO")
plot4 <- FeatureScatter(L3, feature1 = "nFeature_HTO", feature2 = "nCount_HTO")
CombinePlots(plots = list(plot3, plot4))
dev.off()

# Removing unwanted cells from the data based on above QC plots
L3 <- subset(L3, subset = nFeature_RNA > 200 & nFeature_RNA < 15000 & percent.mt < 25)
...
L3 <- SCTransform(L3, vars.to.regress = "percent.mt", verbose = TRUE)

# finding variable genes
L3 <- FindVariableFeatures(L3, selection.method = "vst", nfeatures = 2000)

# Identify the 10 most highly variable genes
top10 <- head(VariableFeatures(L3), 10)

# plot variable features with and without labels
pdf(file.path(figures_dir, "Variable.Genes.pdf"),width=10,height=4,paper='special')
plot1 <- VariableFeaturePlot(L3)
...
set.seed(1)
L3 <- RunPCA(L3, verbose = FALSE)
pdf(file.path(figures_dir, "Elbowplot.pdf"),width=7,height=5,paper='special')
ElbowPlot(L3)
dev.off()

print(L3[["pca"]], dims = 1:5, nfeatures = 5)


pdf(file.path(figures_dir, "Dim.plot.pca.pdf"),width=5,height=4,paper='special')
DimPlot(L3, reduction = "pca")
dev.off()

# Cluster the cells

L3 <- FindNeighbors(L3, dims = 1:10)
L3 <- FindClusters(L3, resolution = 1)
head(Idents(L3), 5)

# Run non-linear dimensional reduction (UMAP/tSNE)
L3 <- RunUMAP(L3, dims = 1:10, verbose = TRUE)
...
DimPlot(L3, reduction = "umap", label = TRUE, split.by = "hash.ID")
dev.off()

# find markers for every cluster compared to all remaining cells, report only the positive ones
L3.markers <- FindAllMarkers(
  L3, only.pos = TRUE, 
  min.pct = 0.25, 
...
DoHeatmap(L3, features = top10$gene) + 
...
ggplot(L3@meta.data, aes(x=hash.ID, fill=seurat_clusters)) + 
...
ggplot(L3@meta.data, aes(seurat_clusters, group = hash.ID)) + 
...
FeaturePlot(L3, features = "IQCD")
dev.off()

pdf(file.path(figures_dir, "Featureplot_TOP2A.pdf"),width=5,height=5,paper='special')
FeaturePlot(L3, features = "TOP2A")
dev.off()

DefaultAssay(L3) <- "SCT"
pdf(file.path(figures_dir, "Featureplot_VHL.pdf"),width=6,height=6,paper='special')
FeaturePlot(L3, features = "VHL") + 
...
counts_matrix <- as.data.frame(L3@assays$RNA$counts)
meta <- L3@meta.data

annot <- as.data.frame(L3@meta.data$hash.ID)
colnames(annot) <- c('V1')
...
# Create the infercnv object with all chr included
...
infercnv_obj = infercnv::run(infercnv_obj,
                             ...
                             out_dir=file.path(base_dir, "/DATA/InfCNV/L3/CNV_output"),
                             output_format="pdf",
...
infercnv_obj = infercnv::run(infercnv_obj,
                             cutoff=0.1, # cutoff=1 works well for Smart-seq2, and cutoff=0.1 works well for 10x Genomics
                             out_dir=file.path(base_dir, "/DATA/InfCNV/L3/CNV_HMM_output"),
                             output_format="pdf",
                             cluster_by_groups=TRUE, 
                             denoise=TRUE,
                             HMM=TRUE,
...
write.table(
  cnvRes,
  file = file.path(base_dir, "DATA", "InfCNV", "L3", "CNV_HMM_output", "L3_cnvRes.csv"),
  sep = "\t",
  row.names = TRUE,
  col.names = TRUE,
  quote = FALSE
...
L3 = infercnv::add_to_seurat(
  infercnv_output_path=file.path(base_dir, "/DATA/InfCNV/L3/CNV_HMM_output"),
  seurat_obj=L3, # optional
  top_n=20
  )
...
FeaturePlot(L3 , features="has_loss_chr3") + 
  ggplot2::scale_colour_gradient(low="lightgrey", high="blue", limits=c(0,1))
FeaturePlot(L3 , features="has_dupli_chr3") + 
  ggplot2::scale_colour_gradient(low="lightgrey", high="blue", limits=c(0,1))


save.image(file = "L3.RData")
saveRDS(L3, file = "L3.rds")
...
## extract annnotation
HMM_anno <- colnames(L3@meta.data[, 19:257])
Chr3_changes <- c(colnames(L3@meta.data[, 37:45]))
Chr3_changes               
Chr5_changes <- c(colnames(L3@meta.data[, 55:63]))
Chr5_changes               

                  
pdf(file.path(figures_dir, "Featureplot_CNV_3p.pdf"),
    width=12,height=10,paper='special')
FeaturePlot(L3, features = Chr3_changes)
dev.off()

pdf(file.path(figures_dir, "Featureplot_CNV_5q.pdf"),
    width=12,height=10,paper='special')
FeaturePlot(L3, features = Chr5_changes)
dev.off()

pdf(file.path(figures_dir, "Featureplot_CNV_3p_5q.pdf"),
    width=16,height=16,paper='special')
FeaturePlot(L3, features = c(Chr3_changes, Chr5_changes))
dev.off()
...
# Initialize the Seurat object with the raw (non-normalized data).
df <- CreateSeuratObject(counts = L3.data$"Gene Expression", project = "MULTI-Chr")

# add ADT/HTO/BC objects
df$HTO <- CreateAssayObject(counts = L3.data$"Antibody Capture")

L3.HTOs <- t(as.data.frame(df$"HTO"@counts))
L3.HTOs <- t(L3.HTOs[,7:12]) #subset only R2 reads for cell group demultiplexing
df[["HTO"]] <- CreateAssayObject(counts = (L3.HTOs+0.1)*10) # create HTO assay with only R2 reads counts 
df[["MULTI"]] <- CreateAssayObject(counts = (L3.HTOs)) #create MULTI-seq assay based on raw HTO counts with only R2 reads
rm(L3.HTOs)

str(df)

# do MULTIseqDemux on L3 df HTO assay (contains both R1 and R2)
L3 <- MULTIseqDemux(
  L3,
  assay = "HTO",
  quantile = 0.9,
...
table(L3$MULTI_ID)
table(L3$MULTI_classification)

# Re-code ID by combining R1 classification and R2 classification. 
L3@meta.data$MULTIdemuxR1R2 = recode(
  L3@meta.data$MULTI_ID, 
  "R1-BC-ID1" = "BC-ID1", 
  "R2-BC-ID1" = "BC-ID1", 
  "R1-BC-ID2" = "BC-ID2", 
  "R2-BC-ID2" = "BC-ID2",
...
table(L3$MULTIdemuxR1R2)


######################################################
#    select Chr3 cells highest scaled_cnv            # 
######################################################
Idents(L3) <- "seurat_clusters"
DimPlot(L3)

Idents(L3) <- "proportion_scaled_cnv_chr3"
DimPlot(L3)

# Select cells with the highest 3p loss with very stringent cut off (>0.15)
L3.3ploss = subset(L3, idents = "0", invert = TRUE)
L3.3ploss@meta.data$Chr3p.loss <- ifelse(
  L3.3ploss@meta.data$proportion_scaled_cnv_chr3 > 0.15, "TRUE", "FALSE"
  )
meta.3ploss = L3.3ploss@meta.data
table(meta.3ploss$Chr3p.loss)


Idents(L3.3ploss) <- "Chr3p.loss"
DimPlot(L3.3ploss, split.by = "HTO_maxID")
...
Idents(L3) <- "proportion_scaled_cnv_chr5"
DimPlot(L3)

# select cells with highest chr5 cnv with very stringent cut off (>0.2)
L3.5q.gain = subset(L3, idents = "0", invert = TRUE)
L3.5q.gain@meta.data$Chr5q.gain <- ifelse(
  L3.5q.gain@meta.data$proportion_scaled_cnv_chr5 > 0.2, "TRUE", "FALSE"
  )
meta.5q.gain = L3.5q.gain@meta.data
meta.5q.gain$Chr5q.gain

Idents(L3.5q.gain) <- "Chr5q.gain"
DimPlot(L3.5q.gain, split.by = "HTO_maxID")
...
FeaturePlot(L3.3ploss, features = c("GBE1", "ROBO1", "ROBO2", "ZNF717"))
dev.off()
...
FeaturePlot(L3, features = "cnv_avg")
L3.clean <- subset(L3, subset = cnv_avg < 0.05)

L3.clean$cnv.class <- "cnv_avg.normal"
L3.clean$cnv.class[L3.clean$cnv_avg > 0.03] <- "cnv_avg.high"
L3.clean$cnv.class[L3.clean$cnv_avg > 0.01 & L3.clean$cnv_avg <= 0.03] <- "cnv_avg.mid"
L3.clean$cnv.class[L3.clean$cnv_avg > 0.00 & L3.clean$cnv_avg <= 0.01] <- "cnv_avg.low"

L3.clean$HTO_recode <- recode(
  L3.clean$HTO_maxID, 
  "R2-BC-ID1" = "01_ID1", 
  "R2-BC-ID2" = "02_ID2", 
  "R2-BC-ID3" = "03_ID3", 
...
Idents(L3.clean) <- "HTO_recode"
Idents(L3.clean) <- factor(x = Idents(L3.clean), levels = sort(levels(L3.clean))) # sort idents

Idents(L3.clean) <- "HTO_maxID"
Idents(L3.clean) <- factor(x = Idents(L3.clean), levels = sort(levels(L3.clean))) # sort idents

table(L3.clean$cnv.class)
pdf(file.path(figures_dir, "VlnPlot_L3.clean.VHL_cnv.class.pdf"),width=8,height=4,paper='special')
VlnPlot(L3.clean, feature = "VHL", split.by = "cnv.class")
dev.off()

#==  re select Chr3 cells highest scaled_cnv  ==# 

## Identify cells with highest chr3 changes
Idents(L3.clean) <- "seurat_clusters"
DimPlot(L3.clean)

Idents(L3.clean) <- "proportion_scaled_cnv_chr3"
DimPlot(L3.clean)

# Select cells with the highest 3p loss with very stringent cut off (>0.2)
L3.clean.3ploss = subset(L3.clean, idents = "0", invert = TRUE)
Idents(L3.clean.3ploss) <- "proportion_scaled_cnv_chr3"
DimPlot(L3.clean.3ploss)
L3.clean.3ploss@meta.data$Chr3p.loss <- ifelse(
  L3.clean.3ploss@meta.data$proportion_scaled_cnv_chr3 > 0.2, 
  "TRUE", "FALSE"
  )
meta.3ploss = L3.clean.3ploss@meta.data
table(meta.3ploss$Chr3p.loss)
Idents(L3.clean.3ploss) <- "Chr3p.loss"
DimPlot(L3.clean.3ploss)
DimPlot(L3.clean.3ploss, split.by = "HTO_maxID")

pdf(file.path(figures_dir, "DimPlot_L3.clean.3ploss_BC_ID.pdf"),
    width=16,height=4,paper='special')
DimPlot(L3.clean.3ploss, split.by = "HTO_maxID")
dev.off()
...
Idents(L3.clean) <- "proportion_scaled_cnv_chr5"
DimPlot(L3.clean)

# select cells with highest chr5 cnv with very stringent cut off (>0.2)
L3.clean.5q.gain = subset(L3.clean, idents = "0", invert = TRUE)
L3.clean.5q.gain@meta.data$Chr5q.gain <- ifelse(
  L3.clean.5q.gain@meta.data$proportion_scaled_cnv_chr5 > 0.2, 
  "TRUE", "FALSE"
  )
meta.5q.gain = L3.clean.5q.gain@meta.data
meta.5q.gain$Chr5q.gain

Idents(L3.clean.5q.gain) <- "Chr5q.gain"
DimPlot(L3.clean.5q.gain, split.by = "HTO_maxID")

pdf(file.path(figures_dir, "DimPlot_L3.clean.5q.gain_BC_ID.pdf"),
    width=16,height=4,paper='special')
DimPlot(L3.clean.5q.gain, split.by = "HTO_maxID")
dev.off()


Idents(L3.clean.5q.gain) <- "proportion_scaled_cnv_chr5"
pdf(file.path(figures_dir, "DimPlot_L3.clean.5q.gain_proportion_scaled_cnv_chr5.pdf"),
    width=16,height=4,paper='special')
DimPlot(L3.clean.5q.gain, split.by = "HTO_maxID")
dev.off()

# 5q cut site coordinates: 98,852,808. The cutting should happen between these two genes
FeaturePlot(L3.clean.5q.gain, feature = c("CAST", "CHD1")) 

Idents(L3.clean.5q.gain) <- "HTO_maxID"
Idents(L3.clean.5q.gain) <- factor(x = Idents(L3.clean.5q.gain), 
                                       levels = sort(levels(L3.clean.5q.gain))
                                       )
Idents(L3.clean.3ploss) <- "HTO_maxID"
Idents(L3.clean.3ploss) <- factor(x = Idents(L3.clean.3ploss), 
                                      levels = sort(levels(L3.clean.3ploss))
                                      )

pdf(file.path(figures_dir, "VlnPlot_L3.clean.5q.gain_CAST.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean.5q.gain, feature = "CAST", split.by = "proportion_scaled_cnv_chr5")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L3.clean.5q.gain_CHD1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean.5q.gain, feature = "CHD1", split.by = "proportion_scaled_cnv_chr5")
dev.off()


VlnPlot(L3.clean.5q.gain, feature = "VHL", split.by = "proportion_scaled_cnv_chr5")

pdf(file.path(figures_dir, "VlnPlot_L3.clean.5q.gain_VHL_by.proportion_scaled_cnv_chr5.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean, feature = "VHL", split.by = "proportion_scaled_cnv_chr5")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L3.clean.5q.gain_VHL_by.cnv.class.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean, feature = "VHL", split.by = "cnv.class")
dev.off()
...
pdf(file.path(figures_dir, "VlnPlot_L3.clean.proportion_scaled_cnv_chr5_SQSTM1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean, feature = "SQSTM1", split.by = "proportion_scaled_cnv_chr5")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L3.clean.proportion_scaled_cnv_chr3_SQSTM1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean, feature = "SQSTM1", split.by = "proportion_scaled_cnv_chr3")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L3.clean.proportion_scaled_cnv_chr5_VHL.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean, feature = "VHL", split.by = "proportion_scaled_cnv_chr5")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L3.clean.proportion_scaled_cnv_chr3_VHL.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean, feature = "VHL", split.by = "proportion_scaled_cnv_chr3")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L3.clean.has_dupli_chr5_VHL.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean, feature = "VHL", split.by = "has_dupli_chr5")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L3.clean.has_loss_chr3_VHL.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean, feature = "VHL", split.by = "has_loss_chr3")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L3.clean.has_dupli_chr5_SQSTM1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean, feature = "SQSTM1", split.by = "has_dupli_chr5")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L3.clean.has_loss_chr3_SQSTM1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean, feature = "SQSTM1", split.by = "has_loss_chr3")
dev.off()

### good plots ====
pdf(file.path(figures_dir, "VlnPlot_L3.clean.proportion_scaled_dupli_chr5_SQSTM1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean, feature = "SQSTM1", split.by = "proportion_scaled_dupli_chr5")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L3.clean.proportion_scaled_loss_chr3_SQSTM1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean, feature = "SQSTM1", split.by = "proportion_scaled_loss_chr3")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L3.clean.proportion_scaled_loss_chr5_SQSTM1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean, feature = "SQSTM1", split.by = "proportion_scaled_loss_chr5")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L3.clean.proportion_scaled_dupli_chr3_SQSTM1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean, feature = "SQSTM1", split.by = "proportion_scaled_dupli_chr3")
dev.off()


VlnPlot(L3.clean, feature = "BAP1", split.by = "proportion_scaled_dupli_chr5")
VlnPlot(L3.clean, feature = "FANCD2", split.by = "proportion_scaled_dupli_chr5")


# ====  recode chr3.loss and chr5.dupli to simply the plotting

L3.clean$Chr3.loss <- ifelse(
  L3.clean@meta.data$proportion_scaled_loss_chr3 > 0.2, 
  "Chr3.loss_TRUE", "Chr3.loss_FALSE"
  )
L3.clean$Chr5.dupli <- ifelse(
  L3.clean@meta.data$proportion_scaled_dupli_chr5 > 0.2, 
  "Chr5.dupli_TRUE", "Chr5.dupli_FALSE"
  )

Idents(L3.clean) <- "HTO_maxID"
Idents(L3.clean) <- factor(x = Idents(L3.clean), levels = sort(levels(L3.clean)))

pdf(file.path(figures_dir, "VlnPlot_L3.clean.Chr3.loss_SQSTM1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean, feature = "SQSTM1", split.by = "Chr3.loss")
dev.off()

pdf(file.path(figures_dir, "VlnPlot_L3.clean.Chr5.dupli_SQSTM1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean, feature = "SQSTM1", split.by = "Chr5.dupli")
dev.off()
...
Idents(L3.clean) <- "Chr3.loss"
Idents(L3.clean) <- factor(x = Idents(L3.clean), levels = sort(levels(L3.clean)))

table(Idents(L3.clean))
DEG_Chr3.loss <- FindMarkers(L3.clean, ident.1 = c('Chr3.loss_TRUE'), ident.2 = c('Chr3.loss_FALSE'), min.pct = 0.25)
write.csv(DEG_Chr3.loss, file = file.path(figures_dir, "DEG_Chr3.loss_DEG.csv"))

pdf(file.path(figures_dir, "Volcano_L3.clean.DEG.Chr3.loss.pdf"),
    width=6,height=4,paper='special')
...
pdf(file.path(figures_dir, "Volcano_L3.clean.DEG.Chr3.loss_INHBA.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean, feature = "INHBA", split.by = "Chr3.loss")
dev.off()

pdf(file.path(figures_dir, "Volcano_L3.clean.DEG.Chr3.loss_SOD3.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean, feature = "SOD3", split.by = "Chr3.loss")
dev.off()

## Identify global DE genes by comparing cell with/without Chr5.dupli
Idents(L3.clean) <- "Chr5.dupli"
Idents(L3.clean) <- factor(x = Idents(L3.clean), levels = sort(levels(L3.clean)))

table(Idents(L3.clean))
DEG_Chr5.dupli <- FindMarkers(L3.clean, ident.1 = c('Chr5.dupli_TRUE'), ident.2 = c('Chr5.dupli_FALSE'), min.pct = 0.25)
write.csv(DEG_Chr5.dupli, file = file.path(figures_dir, "DEG_Chr5.dupli_DEG.csv"))

# draw volcano plot
pdf(file.path(figures_dir, "Volcano_L3.clean.DEG.Chr5.dupli.pdf"),
    width=6,height=4,paper='special')
...
pdf(file.path(figures_dir, "Volcano_L3.clean.DEG.Chr5.dupli_ANKHD1.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean, feature = "ANKHD1", split.by = "Chr5.dupli")
dev.off()

pdf(file.path(figures_dir, "Volcano_L3.clean.DEG.Chr5.dupli_TAF7.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean, feature = "TAF7", split.by = "Chr5.dupli")
dev.off()

pdf(file.path(figures_dir, "Volcano_L3.clean.DEG.Chr5.dupli_TARBP2.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean, feature = "TARBP2", split.by = "Chr5.dupli")
dev.off()

pdf(file.path(figures_dir, "Volcano_L3.clean.DEG.Chr5.dupli_G3BP.pdf"),
    width=6,height=4,paper='special')
VlnPlot(L3.clean, feature = "G3BP1", split.by = "Chr5.dupli")
dev.off()
...
Idents(L3.clean) <- "seurat_clusters"
L3.clean.3p5qcnv = subset(L3.clean, idents = "0", invert = TRUE)
L3.clean.3p5qcnv@meta.data$cnv.3p5q <- ifelse(
  L3.clean.3p5qcnv$Chr3.loss == "TRUE" & 
    L3.clean.3p5qcnv$Chr5.dupli == "TRUE", "TRUE", "FALSE"
  )

table(L3.clean.3p5qcnv$Chr3.loss)
table(L3.clean.3p5qcnv$Chr5.dupli)

L3.clean.3p5qcnv.meta <- L3.clean.3p5qcnv@meta.data
filtered_cells <- cbind(L3.clean.3p5qcnv$Chr3.loss, L3.clean.3p5qcnv$Chr5.dupli)
filtered_cells.3p5qcnv.selected <- rownames(
  L3.clean.3p5qcnv.meta)[L3.clean.3p5qcnv.meta$Chr3.loss
                             == "TRUE" & L3.clean.3p5qcnv.meta$Chr5.dupli 
                             == "TRUE"]

# ==== using less stringent cut offs
L3.clean$Chr3loss.Chr5.dupli <- ifelse(
  L3.clean@meta.data$proportion_scaled_loss_chr3 > 0 &
  L3.clean@meta.data$proportion_scaled_dupli_chr5 > 0, 
  "Chr3loss.Chr5.dupli_TRUE", "Chr3loss.Chr5.dupli_FALSE"
  )

table(L3.clean$Chr3loss.Chr5.dupli)
...
kp <- plotKaryotype(plot.type=2, L3somes = c("chr3"))
...
pdf(file.path(figures_dir, "Karyotype_L3.clean.DEG.Chr3.pdf"),
...
pdf(file.path(figures_dir, "Karyotype_L3.clean.DEG.Chr3_top50.pdf"),
    width=30,height=30,paper='special')
...
pdf(file.path(figures_dir, "Karyotype_L3.clean.DEG.Chr3_top50_AllL3.pdf"),
    width=30,height=30,paper='special')
...
# === Only plot Chr3  ===
pdf(file.path(figures_dir, "Karyotype_L3.clean.DEG.Chr3_only_top50.pdf"),
    width=10,height=8,paper='special')

kp <- plotKaryotype(plot.type=2, plot.params = pp, L3somes = c("chr3"))
kpAddMainTitle(kp, main = "Gene expression - Chr3 loss True vs False")
## Data panel 1
kpPoints(kp, data=sign.genes, y=sign.genes$avg_log2FC, cex=cex.val, ymax=fc.ymax, ymin=fc.ymin, r1=points.top, col=sign.col)
gene.mean <- start(top.genes) + (end(top.genes) - start(top.genes))/2
...
# establish karyotype plot
kp <- plotKaryotype(plot.type=2, L3somes = c("chr5"))
kpDataBackground(kp, data.panel = 1, col="#AACBFF")
kpDataBackground(kp, data.panel = 2, col="#FFAACB")
...
pdf(file.path(figures_dir, "Karyotype_L3.clean.DEG.chr5.pdf"),
    width=30,height=30,paper='special')
kp <- plotKaryotype(plot.type=2)
kpDataBackground(kp, data.panel = 1, col="#AACBFF")
kpPoints(kp, data=sign.genes, y=sign.genes$avg_log2FC, ymax=fc.ymax, ymin=fc.ymin)
...
# add top 50 gene symbol
pdf(file.path(figures_dir, "Karyotype_L3.clean.DEG.chr5_top50.pdf"),
    width=30,height=30,paper='special')
...
pdf(file.path(figures_dir, "Karyotype_L3.clean.DEG.Chr5_top50_AllChrom2.pdf"),
    width=30,height=30,paper='special')
...
# === Only plot chr5  ===
pdf(file.path(figures_dir, "Karyotype_L3.clean.DEG.chr5_only_top50.pdf"),
    width=10,height=8,paper='special')

kp <- plotKaryotype(plot.type=2, plot.params = pp, L3somes = c("chr5"))
kpAddMainTitle(kp, main = "Gene expression - chr5 dupli True vs False")
## Data panel 1
kpPoints(kp, data=sign.genes, y=sign.genes$avg_log2FC, cex=cex.val, ymax=fc.ymax, ymin=fc.ymin, r1=points.top, col=sign.col)
gene.mean <- start(top.genes) + (end(top.genes) - start(top.genes))/2
